...
//Class of the Mob Skeleton but he's an enemy within the Volcano area. 
class Skeleton : public Enemy {
public:
	//".png" is referenced to the tsuna's spritesheet that I re-edited so that it would work. 
	Skeleton(Map *map, float x, float y) : Enemy(map, "skeletonspritesheet.png", x, y, sf::IntRect(0, 0, 80, 70)) { 
		this->health = 6; 
		this->velocity = sf::Vector2f(-0.6f, 0);
		//this->Animate(0, 4); //atk animation
		this->SetFrame(4);
		this->Animate(4, 12); //walk animation
		this->yOffset = -4; //determines the y offset of the skeleton's positioning. 

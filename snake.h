...
//Class of the Mob snake lady but she's an enemy within the Desert area. 
class Snake : public Enemy {
public:
	//".png" is referenced to the tsuna's spritesheet that I re-edited so that it would work. 
	Snake(Map *map, float x, float y) : Enemy(map, "snakespritesheet.png", x, y, sf::IntRect(0, 0, 90, 65)) { 
		this->velocity = sf::Vector2f(-0.8f, 0);
		this->health = 5; //5 health in total 
		
		//this->Animate(0, 4); //death animation
		//this->Animate(4, 12); //atk animation
		this->SetFrame(12);
		this->Animate(12, 17);  //walking animations
		this->yOffset = -1; //determines the y offset of the snake's positioning. 
		this->fps = 16 * 10; //slows down the animations 
	}
	...
	bool Update(sf::RenderWindow *window) {
		...
		else if (this->health <= 0 && this->currentFrame > 12) { //If I wanted a death anim, these are the things I have to do: this->health <= 0 && this->currentFrame < 15

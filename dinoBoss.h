...
//Class of the Boss Dino but he's an enemy within the Forest area. 
class DinoBoss : public Enemy {
public:
	//".png" is referenced to the tsuna's spritesheet that I re-edited so that it would work. 
	DinoBoss(Map *map, float x, float y) : Enemy(map, "dinodefault.png", x, y, sf::IntRect(0, 0, 33, 65)) { //HAVENT ANIMATED OR ADD THIS MOB TO THE MAP.CPP PART YET
		this->velocity = sf::Vector2f(-0.5f, 0);
		this->health = 15; //15 health in total

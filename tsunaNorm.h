...
//Class of Tsuna's norm form but he's an enemy within the Forest area. 
class TsunaNorm : public Enemy {
public:
	//".png" is referenced to the tsuna's spritesheet that I re-edited so that it would work. 
	TsunaNorm(Map *map, float x, float y) : Enemy(map, "tsunanormspritesheet.png", x, y, sf::IntRect(0, 0, 60, 70)) { 
		this->health = 2; 
		this->velocity = sf::Vector2f(-0.8f, 0);
		this->Animate(0, 8); //I need to come back and animate this wolf later! 
		this->yOffset = -2; //determines the y offset of tsuna's y position
	...
	bool Update(sf::RenderWindow *window) {
		...
		else if (this->health <= 0 && this->currentFrame < 8) { //If I wanted a death anim, these are the things I have to do: this->health <= 0 && this->currentFrame < 15
